...
for x, y in data:
    ...
    R.append(r)
    FI.append(phi)
R = np.append(R, R[0])            # Дублируем радиус первой точки в конец
FI = np.append(FI, FI[0] + 2*np.pi) # Дублируем угол первой точки + полный оборот
FI = np.array(FI)
R = np.array(R)
...
# 3. Интерполяция и производные с помощью CubicSpline
# bc_type='periodic' обеспечивает, что производные в начале и конце совпадут
...
# 4. Получение значений (пример для графика с большим числом точек)
