...
class Kulachok_polidain:
    ...
    def display_graphs(self, N = 1000):
        ...
        H = self.fun_h(fi_list)
        V = self.fun_v(fi_list)
        A = self.fun_a(fi_list)
        D = self.fun_d(fi_list)
        K = self.fun_k(fi_list)
...
if __name__ == '__main__':
    #import Polidain.matplotlib_settings.profile_1
    ...
    k_2 = 30 # коэффициент агрессивности второго участка (Фаза подъёма)
    k_3 = 30 # коэффициент агрессивности четвёртого участка (Фаза опускания)
    k_4 = 30 # коэффициент агрессивности пятого участка (Фаза выбора зазора)
    try:
